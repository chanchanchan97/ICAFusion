...
class YOLOV5TorchObjectDetector(nn.Module):
    ...
    @staticmethod
    def non_max_suppression(prediction, logits, conf_thres=0.001, iou_thres=0.5, classes=None, agnostic=False,
    ...
    def forward(self, img_vis, img_ir):
        prediction, logits, _ = self.model(img_vis, img_ir, augment=False)
        prediction, logits = self.non_max_suppression(prediction, logits, self.confidence, self.iou_thresh,
                                                      classes=None,
                                                      agnostic=self.agnostic)
        self.boxes, self.class_names, self.classes, self.confidences = [[[] for _ in range(img_vis.shape[0])] for _ in range(4)]
    ...
    def preprocessing(self, img_vis, img_ir):
        if len(img_vis.shape) != 4:
            img_vis = np.expand_dims(img_vis, axis=0)
        if len(img_ir.shape) != 4:
            img_ir = np.expand_dims(img_ir, axis=0)
        im0_vis, im0_ir = img_vis.astype(np.uint8), img_ir.astype(np.uint8)
        img_vis = np.array([self.yolo_resize(im_vis, new_shape=self.img_size)[0] for im_vis in im0_vis])
        img_ir = np.array([self.yolo_resize(im_ir, new_shape=self.img_size)[0] for im_ir in im0_ir])
        img_vis, img_ir = img_vis.transpose((0, 3, 1, 2)), img_ir.transpose((0, 3, 1, 2))
        img_vis, img_ir = np.ascontiguousarray(img_vis), np.ascontiguousarray(img_ir)
        img_vis, img_ir = torch.from_numpy(img_vis).to(self.device), torch.from_numpy(img_ir).to(self.device)
        img_vis, img_ir = img_vis / 255.0, img_ir / 255.0
